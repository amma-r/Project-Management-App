import { useState } from 'react'
import './App.css'
import Content from './components/Content'
import NewProject from './components/NewProject'
import Sidebar from './components/Sidebar'
// import NoProjectSelect from './components/NoProjectSelect'

function App() {
  const [showNewProj, setShowNewProj] = useState(false)
  const [showContent, setShowContent] = useState(true)
  const [sidebarProjects, setSidebarProjects] = useState([])
  const [selectedProject, setSelectedProject] = useState({})

  // if (sidebarProjects.length > 0) {
  //   setSelectedProject(sidebarProjects[0])
  // }
  // console.log("selected Project: ", selectedProject)
  // console.log("sidebar project array: ", sidebarProjects)

  function showNewProjHandler(state) {
    if (state) {
      setSelectedProject({})
      setShowNewProj(true)
      setShowContent(false)
    }
    else {
      setShowNewProj(false)
      setShowContent(true)
    }
  }

  return (
    <>
      <main className="h-screen my-8 flex">
        <Sidebar showNewProjHandler={showNewProjHandler} sidebarProjects={sidebarProjects} setSelectedProject={setSelectedProject} selectedProject={selectedProject} />
        {showContent && <Content selectedProject={selectedProject} showNewProjHandler={showNewProjHandler} setSidebarProjects={setSidebarProjects} sidebarProjects={sidebarProjects}/>}
        {showNewProj && <NewProject setSidebarProjects={setSidebarProjects} showNewProjHandler={showNewProjHandler} />}
      </main>
    </>
  )
}

export default App
{/* 
      <h2 className="text-xl font-bold text-stone-700 my-4">My Learning</h2>
      <p className="text-stone-600 mb-4">...</p>
      <div className="w-[35rem] mt-16">...</div>
      <button className="text-stone-800 hover:text-stone-950">...</button>
      <button className="px-6 py-2 rounded-md bg-stone-800 text-stone-50 hover:bg-stone-950">...</button>
      <div className="flex items-center gap-4"> </div>
      <input className="w-64 px-2 py-1 rounded-sm bg-stone-200" />
      <button className="text-stone-700 hover:text-stone-950">...</button>
      <div className="mt-24 text-center w-2/3"> </div>
      <img className="w-16 h-16 object-contain mx-auto" />
      <h2 className="text-xl font-bold text-stone-500 my-4">...</h2>
      <p className="text-stone-400 mb-4">...</p>
      <p className="mt-8">...</p>
      <h2 className="mb-8 font-bold uppercase md:text-xl text-stone-200">...</h2>
      <header className="pb-4 mb-4 border-b-2 border-stone-300">...</header>
      <div className="flex items-center justify-between">...</div>
      <button className="text-stone-600 hover:text-stone-950">...</button>
      <p className="mb-4 text-stone-400">...</p>
      <p className="text-stone-600 whitespace-pre-wrap">...</p>
      <h2 className="text-2xl font-bold text-stone-700 mb-4">Tasks</h2>
      <p className="text-stone-800 my-4">...</p>
      <ul className="p-4 mt-8 rounded-md bg-stone-100">...</ul>
      <li className="flex justify-between my-4">...</li>
      <button className="w-full text-left px-2 py-1 rounded-sm my-1 hover:text-stone-200 hover:bg-stone-800">hello</button>
    */}